export interface OwnedSpaceDevice {
  ...
  /** SPKI-encoded key used to authorize subsequent membership changes. */
  signingKey: string;
}

export interface SpaceMembershipUpdate {
  format: 1;
  sequence: number;
...
export interface SpaceDeviceMembershipTrust {
  genesisKey: string;
  knownHead: string;
  updates: SpaceMembershipUpdate[];
}
...
const validateMembership = (devices: readonly OwnedSpaceDevice[]) => {
...
const validateMembershipTransition = (previous: readonly OwnedSpaceDevice[], next: readonly OwnedSpaceDevice[]) => {
...
/** A full membership snapshot signed by the device authorized in the preceding snapshot. */
export async function signSpaceMembershipUpdate(subtle: SubtleCrypto, key: CryptoKey,
  value: Pick<SpaceMembershipUpdate, "sequence" | "previous" | "signer" | "devices" | "recoveryKey">): Promise<SpaceMembershipUpdate> {
  validateMembership(value.devices);
  if (!Number.isSafeInteger(value.sequence) || value.sequence < 1 || !value.signer ||
    (value.sequence === 1) !== (value.previous === null)) throw new Error("Invalid space device membership sequence.");
...
export async function verifySpaceDeviceMembership(subtle: SubtleCrypto, updates: readonly SpaceMembershipUpdate[],
  genesisKey: string, knownHead?: string): Promise<{ devices: OwnedSpaceDevice[]; hash: string }> {
  if (!updates.length || updates.length > 10000) throw new Error("Invalid space device membership history.");
  ...
  for (const [index, update] of updates.entries()) {
    if (update.format !== 1 || update.sequence !== index + 1 || update.previous !== previous ||
      typeof update.signer !== "string" || !update.signer) throw new Error("Invalid space device membership ancestry.");
    validateMembership(update.devices);
    if (index > 0) validateMembershipTransition(devices, update.devices);
    if (index === 0) recoveryKey = update.recoveryKey;
    else if (update.recoveryKey !== recoveryKey) throw new Error("Space device membership recovery authority changed.");
    ...
    if (update.signer === "recovery" && index > 0 && recoveryKey) trusted = recoveryKey;
      ...
      throw new Error("Space device membership signer is not authorized.");
    } else trusted = signer.signingKey;
    const bytes = updateBytes(unsignedUpdate(update));
    const hash = encode(new Uint8Array(await subtle.digest("SHA-256", bytes)));
    if (hash !== update.hash) throw new Error("Space device membership hash mismatch.");
    let valid = false;
    try {
    ...
    if (!valid) throw new Error("Space device membership signature is invalid.");
    knownHeadSeen ||= hash === knownHead;
    previous = hash;
    devices = update.devices;
  }
  if (knownHead && !knownHeadSeen) throw new Error("Space device membership rollback or fork detected.");
  return { devices, hash: previous! };
}

export function settingsFolderDevices(devices: readonly OwnedSpaceDevice[]): string[] {
  validateMembership(devices);
