...
import { chmod, mkdtemp, readdir, readFile, writeFile, rm, stat } from "node:fs/promises";
...
test("node partials survive suspension and only restore the same remote version", async () => {
  ...
  try {
    ...
    const [stagingName] = (await readdir(root)).filter(name => name.startsWith('.syncpeer-download-'));
    assert.ok(stagingName);
    const staleTemporary = path.join(root, stagingName, 'metadata.tmp');
    await writeFile(staleTemporary, 'synthetic stale metadata');
    await chmod(staleTemporary, 0o666);
    await first.suspend!();
    assert.equal((await stat(path.join(root, stagingName, 'metadata.json'))).mode & 0o777, 0o600);
