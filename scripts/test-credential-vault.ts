...
test("vault encrypts folder credentials, copies defaults only on creation, and persists manual lock", async () => {
  ...
  await vault.create("synthetic-master-password");
  await vault.setDefaultPassword("synthetic-default-one");
  await vault.addFolder("fixture-first");
  await vault.addFolder("fixture-imported", "synthetic-import-password");
  await vault.setDefaultPassword("synthetic-default-two");
  await vault.addFolder("fixture-second");
  assert.equal(await vault.folderPassword("fixture-first"), "synthetic-default-one");
  assert.equal(await vault.folderPassword("fixture-imported"), "synthetic-import-password");
  assert.equal(await vault.folderPassword("fixture-second"), "synthetic-default-two");
  ...
  assert.equal(await automatic.folderPassword("fixture-first"), "synthetic-default-one");
  assert.equal((await createCredentialVault(options).initialize()).phase, "locked");
